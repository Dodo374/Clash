...
function main(config) {
  ...
  serviceConfigs.forEach((svc) => {
    if (ruleOptions[svc.key]) {
      ...
      let groupProxies
      if (svc.reject) {
        groupProxies = ['REJECT', '直连', '自动选择']
      } else if (svc.key === 'biliintl' || svc.key === 'bahamut') {
        groupProxies = ['自动选择', '直连', ...regionGroupNames]
      } else {
        groupProxies = ['自动选择', ...regionGroupNames, '直连']
      }
  ...
  functionalGroups.push(
    {
      ...
      name: '下载软件',
      type: 'select',
      proxies: ['直连', 'REJECT', '自动选择', '国内网站', ...regionGroupNames],
    ...
    },
    {
      ...
      name: '其他外网',
      type: 'select',
      proxies: ['自动选择', '国内网站', ...regionGroupNames],
    ...
    },
    {
      ...
      name: '国内网站',
      type: 'select',
      proxies: ['直连', '自动选择', ...regionGroupNames],
